...
- **RDX-114**: The RDX format MUST support `frameworkReferences` on attack steps, enabling structured cross-referencing to external attack frameworks and threat taxonomies (e.g. MITRE ATT&CK for Automotive, EMB3D, Auto-ISAC Threat Matrix, CAPEC). Each framework reference MUST include a `framework` name and `techniqueId`. Optional fields SHOULD include a `url` for deep-linking to the framework entry and a `description` of the relevance to the attack step. This enables traceability between organization-specific TARA data and industry-standard threat catalogs (see GitHub issue #38)
