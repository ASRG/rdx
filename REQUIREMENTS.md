...
- **RDX-114**: Attack feasibility ratings MUST be embeddable directly within `attackSteps` as an inline array, eliminating the need for a separate top-level entity with its own `id`. Embedded ratings are contextually bound to the enclosing step and do not require `id`, `attackStepId`, or `attackPathId` fields. Standard ISO/SAE 21434 inputFactor names are: `elapsedTime`, `expertise`, `knowledge`, `windowOfOpportunity`, and `equipment` (see GitHub issue #39)
- **RDX-115**: The `windowOfOpportunity` input factor MUST be supported in attack feasibility rating `inputFactors` to capture the accessibility and opportunity window for attack execution, per ISO/SAE 21434 feasibility methodology (see GitHub issue #39)
