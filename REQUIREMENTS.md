...
## 14. Profile Configuration Requirements

Profile configurations define which RDX fields are required, optional, or not allowed, enabling organizations and standards bodies to enforce data completeness rules on top of the base JSON Schema validation.

### 14.1 Profile System

- **RDX-114**: The RDX format MUST support a profile configuration system that allows defining per-field requirement levels (`required`, `optional`, `notAllowed`) for any field in an RDX document (see GitHub issue #40)
- **RDX-115**: Each profile MUST declare a `defaultFieldRequirement` that applies to any RDX field not explicitly listed in the profile's `fieldRequirements` map. Valid values are `required`, `optional`, and `notAllowed` (see GitHub issue #40)
- **RDX-116**: Profile field paths MUST use dot notation with `*` as the array-item wildcard (e.g., `riskSet.assets.*.properties`). A path ending at an array section (e.g., `riskSet.attackSteps`) controls whether that section must be present and non-empty (see GitHub issue #40)
- **RDX-117**: Each profile MUST include an `integrity` field containing a cryptographic hash of the profile content (excluding the `integrity` field itself, replaced by `{}`). The hash MUST be computed over the canonical JSON serialization of the profile (keys sorted lexicographically, no extra whitespace, UTF-8 encoded). This allows consuming tools to detect tampering in transit (see GitHub issue #40)
- **RDX-118**: RDX documents MAY include a `profileRef` field referencing the URI or identifier of the profile they claim to conform to. Validation of conformance is the responsibility of the consuming or providing tool, not the JSON Schema validator (see GitHub issue #40)
- **RDX-119**: The RDX repository MUST provide at minimum two reference profiles: (1) `rdx-profile-schema-minimum` requiring only the fields mandated by the JSON Schema, and (2) `rdx-profile-iso21434-standard` requiring all fields identified as mandatory for a complete ISO/SAE 21434 TARA (see GitHub issue #40)
- **RDX-120**: Users MUST be able to define their own custom profiles conforming to `spec/json/rdx-profile.schema.json` to enforce organization-specific or project-specific data completeness requirements (see GitHub issue #40)
